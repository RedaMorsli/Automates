...
export var is_camera_control_enabeled : bool = true
	...
	if event is InputEventMouseButton:
		if event.is_pressed():
			...
			if event.button_index == BUTTON_WHEEL_UP:
				if (self.zoom.x >= 0.3) and (self.zoom.y >= 0.3) and is_camera_control_enabeled:
					self.zoom -= Vector2(0.1, 0.1)
					for node in self.get_children():
						node.rect_scale -= Vector2(0.1, 0.1)
			# zoom out
			if event.button_index == BUTTON_WHEEL_DOWN:
				if (self.zoom.x <= 2.8) and (self.zoom.y <= 2.8) and is_camera_control_enabeled:
					self.zoom += Vector2(0.1, 0.1)
					for node in self.get_children():
							node.rect_scale += Vector2(0.1, 0.1)
